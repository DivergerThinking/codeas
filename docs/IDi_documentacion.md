...
- **Alcance**: se orienta a repositorios multilenguaje y a modelos OpenAI, Anthropic y Gemini. Puede ejecutarse como CLI o a través de la UI en Streamlit, lo que permite validar las capacidades sin código adicional.
...
- Persistencia de filtros y salidas en la carpeta `.codeas`, permitiendo reanudar sesiones con el mismo contexto de forma segura.
...
- Compatibilidad con sesiones sin conexión a internet para revisión (usando metadatos previamente generados), ayudando a entornos con restricciones de red.
...
5. **Control de privacidad**: el filtrado previo evita incluir archivos con datos sensibles en prompts, alineándose con políticas de mínimo privilegio.
...
- Compatibilidad con modo lectura (solo mostrar contexto sin ejecutar generación) para validar el alcance de la información compartida.
...
- **Seguridad operacional**: la herramienta registra los modelos utilizados y los parámetros de temperatura/máximo de tokens, aportando transparencia para auditorías técnicas.
...
- Controles adicionales: se pueden fijar límites de coste y seleccionar modelos más baratos cuando el volumen de contexto es alto.
...
- Mantiene referencias a los archivos origen y a las dependencias externas para facilitar la implementación de los tests.
...
- Permite registrar validaciones posteriores (`terraform plan`/`apply`) como evidencia de conformidad.
...
- Registra qué diffs fueron aplicados y cuáles quedaron descartados, mejorando la trazabilidad en auditorías.
...
- **Operación colaborativa**: la exportación de metadatos y prompts permite que varias personas trabajen sobre el mismo repositorio sin recalcular información, reduciendo coste y tiempo.
...
- Registro de costes por modelo y operación, habilitando comparativas entre proveedores y ayudando a optimizar presupuestos.
...
- **Aprendizaje acumulativo**: los prompts editables y la persistencia de filtros capturan conocimiento organizativo, reduciendo la curva de aprendizaje en proyectos grandes.
...
- **Auditoría de modelos**: la bitácora de uso indica qué modelo y parámetros se emplearon, añadiendo transparencia para equipos de seguridad o arquitectura.

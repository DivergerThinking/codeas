...
class Agent(BaseModel):
    ...
    def run(
        ...
        # Added error handling for LLM client interaction
        try:
            response = llm_client.run(
                messages, model=self.model, response_format=self.response_format
            )
        except Exception as e:
            # Propagate or handle the exception as needed by the application
            # For now, re-raising to indicate failure in LLM interaction
            raise RuntimeError(f"Error during LLM client run: {e}") from e
    ...
    def preview(self, context: Union[dict, list, str]) -> AgentPreview:
        messages = self.get_messages(context)
        # Added error handling for token/cost calculation in preview
        try:
            tokens, cost = self.calculate_tokens_and_cost(messages)
        except Exception as e:
             # Propagate or handle the exception as needed by the application
            raise RuntimeError(f"Error during token/cost calculation preview: {e}") from e
    ...
    def calculate_tokens_and_cost(self, messages: Union[list, dict], response=None):
        # Error handling moved into the helper methods
    ...
    def _sum_get_request_tokens_and_cost(self, responses: dict):
        ...
        for response in responses.values():
            # Error handling for individual response calculation
            try:
                results.append(self._get_request_tokens_and_cost(response))
            except Exception as e:
                # Handle or log error for this specific response calculation
                # Skipping this result allows partial success for batch calls
                pass

        # Ensure results is not empty before summing
        if not results:
             raise RuntimeError("Failed to calculate tokens/cost for any response in batch.")
    ...
    def _get_request_tokens_and_cost(self, response):
        # Added error handling for accessing response attributes and calculations
        try:
            tokens = {
    ...
    def _get_request_tokens_and_cost(self, response):
        ...
        try:
            ...
            # Propagate the exception to the caller (e.g., calculate_tokens_and_cost)
            raise RuntimeError(f"Error calculating request tokens/cost: {e}") from e
    ...
    def _sum_calculate_tokens_and_cost(self, batch_messages: dict, batch_response=None):
        ...
        for key, messages in batch_messages.items():
            ...
            # Error handling for individual batch item calculation
            try:
                 results.append(self._calculate_tokens_and_cost(messages, response))
            except Exception as e:
                 # Handle or log error for this specific message/response pair
                 # Skipping this result allows partial success for batch calls
                 pass

        # Ensure results is not empty before summing
        if not results:
             raise RuntimeError("Failed to calculate tokens/cost for any item in batch.")
    ...
    def _calculate_tokens_and_cost(self, messages: list, response=None):
        # Added error handling for token calculation logic
        try:
            if response is None:
                ...
                # Assuming response is a dict with "content" key based on how it's used in run()
                response_content = response.get("content")
                if response_content is None:
                     raise ValueError("Response object is missing 'content' key")
            ...
            # Propagate the exception to the caller (e.g., calculate_tokens_and_cost)
            raise RuntimeError(f"Error calculating tokens/cost: {e}") from e

...
# Define a constant for the duplicated literal string
FILE_INCLUSION_COLUMN = "Incl."
...
def get_selected_files_info():
    num_selected_files = sum(state.files_data[FILE_INCLUSION_COLUMN])
    total_files = len(state.files_data[FILE_INCLUSION_COLUMN])
    selected_tokens = sum(
        token
        for incl, token in zip(state.files_data[FILE_INCLUSION_COLUMN], state.files_data["Tokens"])
        if incl
    )
...
def display_files_editor():
    ...
    st.data_editor(
        ...
        column_config={
            FILE_INCLUSION_COLUMN: st.column_config.CheckboxColumn(width=5),
...
def sort_files_data():
    sorted_data = sorted(
        zip(
            state.files_data[FILE_INCLUSION_COLUMN],
...
def sort_files_metadata(files_metadata):
    sorted_data = sorted(
        zip(
            files_metadata[FILE_INCLUSION_COLUMN],
            files_metadata["Path"],

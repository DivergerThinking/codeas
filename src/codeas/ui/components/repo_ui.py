...
def get_selected_files_info():
    num_selected_files = sum(state.files_data[COLUMN_INCL_KEY])
    total_files = len(state.files_data[COLUMN_INCL_KEY])
    selected_tokens = sum(
        token
        for incl, token in zip(state.files_data[COLUMN_INCL_KEY], state.files_data["Tokens"])
        if incl
    )
...
def display_files_editor():
    ...
    st.data_editor(
        ...
        column_config={
            COLUMN_INCL_KEY: st.column_config.CheckboxColumn(width=5),
...
def sort_files_data():
    sorted_data = sorted(
        zip(
            state.files_data[COLUMN_INCL_KEY],
...
def sort_files_metadata(files_metadata):
    sorted_data = sorted(
        zip(
            files_metadata[COLUMN_INCL_KEY],
            files_metadata["Path"],

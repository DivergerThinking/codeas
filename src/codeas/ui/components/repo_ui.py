...
def get_selected_files_info():
    num_selected_files = sum(state.files_data[INCLUDED_KEY])
    total_files = len(state.files_data[INCLUDED_KEY])
    selected_tokens = sum(
        token
        for incl, token in zip(state.files_data[INCLUDED_KEY], state.files_data[TOKENS_KEY])
        if incl
    )
...
def display_files_editor():
    ...
    st.data_editor(
        ...
        column_config={
            INCLUDED_KEY: st.column_config.CheckboxColumn(width=5),
            PATH_KEY: st.column_config.TextColumn(width="large"),
            TOKENS_KEY: st.column_config.NumberColumn(width=5),
        },
        disabled=True,
        height=300,
    )
...
def sort_files_data():
    sorted_data = sorted(
        zip(
            state.files_data[INCLUDED_KEY],
            state.files_data[PATH_KEY],
            state.files_data[TOKENS_KEY],
        ),
...
def sort_files_metadata(files_metadata):
    sorted_data = sorted(
        zip(
            files_metadata[INCLUDED_KEY],
            files_metadata[PATH_KEY],
            files_metadata[TOKENS_KEY],
        ),
        key=lambda x: (not x[0], x[1]),
    )
    (

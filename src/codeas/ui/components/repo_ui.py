...
# Define constants for column keys to avoid magic strings and duplication
INCLUSION_COLUMN_KEY = "Incl."
...
def get_selected_files_info():
    num_selected_files = sum(state.files_data[INCLUSION_COLUMN_KEY])
    total_files = len(state.files_data[INCLUSION_COLUMN_KEY])
    selected_tokens = sum(
        token
        for incl, token in zip(state.files_data[INCLUSION_COLUMN_KEY], state.files_data[TOKENS_COLUMN_KEY])
        if incl
    )
...
def display_files_editor():
    ...
    st.data_editor(
        ...
        column_config={
            INCLUSION_COLUMN_KEY: st.column_config.CheckboxColumn(width=5),
...
def sort_files_data():
    sorted_data = sorted(
        zip(
            state.files_data[INCLUSION_COLUMN_KEY],
...
def sort_files_metadata(files_metadata):
    sorted_data = sorted(
        zip(
            files_metadata[INCLUSION_COLUMN_KEY],

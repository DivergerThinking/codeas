...
def get_selected_files_info():
    num_selected_files = sum(state.files_data[INCL_COLUMN_NAME])
    total_files = len(state.files_data[INCL_COLUMN_NAME])
    selected_tokens = sum(
        token
        for incl, token in zip(state.files_data[INCL_COLUMN_NAME], state.files_data[TOKENS_COLUMN_NAME])
        if incl
    )
...
def display_files_editor():
    ...
    st.data_editor(
        ...
        column_config={
            INCL_COLUMN_NAME: st.column_config.CheckboxColumn(width=5),
            PATH_COLUMN_NAME: st.column_config.TextColumn(width="large"),
            TOKENS_COLUMN_NAME: st.column_config.NumberColumn(width=5),
        },
        disabled=True,
        height=300,
    )
...
def sort_files_data():
    sorted_data = sorted(
        zip(
            state.files_data[INCL_COLUMN_NAME],
            state.files_data[PATH_COLUMN_NAME],
            state.files_data[TOKENS_COLUMN_NAME],
        ),
...
def sort_files_metadata(files_metadata):
    sorted_data = sorted(
        zip(
            files_metadata[INCL_COLUMN_NAME],
            files_metadata[PATH_COLUMN_NAME],
            files_metadata[TOKENS_COLUMN_NAME],
        ),
        key=lambda x: (not x[0], x[1]),
    )
    (

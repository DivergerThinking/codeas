...
def get_selected_files_info():
    num_selected_files = sum(state.files_data[INCL_KEY])
    total_files = len(state.files_data[INCL_KEY])
    selected_tokens = sum(
        token
        for incl, token in zip(state.files_data[INCL_KEY], state.files_data[TOKENS_KEY])
        if incl
    )
...
def display_files_editor():
    ...
    st.data_editor(
        ...
        column_config={
            INCL_KEY: st.column_config.CheckboxColumn(width=5),

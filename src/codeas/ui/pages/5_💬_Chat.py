...
def display_config_section():
    with st.expander("CONTEXT", icon="⚙️", expanded=False):
        ...
        if complex_config_selected:
            files_missing_metadata = metadata_ui.display()

        if complex_config_selected and not any(files_missing_metadata):
             ...
             editor_data = files_metadata
             editor_type = "metadata"
        elif not complex_config_selected:
        ...
        if editor_type:
             ...
             if editor_type == "metadata":
                 repo_ui.display_metadata_editor(editor_data)
...
def handle_send_button():
    ...
    for i in range(1, 4):
         ...
         user_input = st.session_state.get(f"instructions{i}", "").strip()
         if template and user_input:
             inputs_to_process.append((template, user_input))

    if not inputs_to_process:
         single_instructions_input = st.session_state.get("instructions", "").strip()
         if single_instructions_input:
            selected_templates_list = [
                st.session_state.get(f"template{i}")
                for i in range(1, 4)
                if st.session_state.get(f"template{i}")
            ]
            single_template = selected_templates_list[0] if len(selected_templates_list) == 1 else ""
            inputs_to_process = [(single_template, single_instructions_input)]


    if inputs_to_process:
        for template, user_input in inputs_to_process:
...
def _handle_streaming_completion(llm_client, messages):
    completion = st.write_stream(llm_client.stream(messages))
    return completion

def _handle_non_streaming_completion(llm_client, messages):
...
def run_agent(model):
    ...
    if (model in ["claude-3-5-sonnet", "claude-3-haiku"] and
    ...
    if model in ["o1-preview", "o1-mini"]:
        completion = _handle_non_streaming_completion(llm_client, messages)
    else:
        completion = _handle_streaming_completion(llm_client, messages)
...
def get_retriever_args():
    ...
    return {
        ...
        "use_full_content": content_types == FULL_CONTENT,
    }
